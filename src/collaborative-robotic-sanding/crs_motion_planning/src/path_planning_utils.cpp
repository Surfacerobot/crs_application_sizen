...
bool loadPathPlanningConfig(const std::string& yaml_fp, pathPlanningConfig& motion_planner_config)
{
  ...
  try
  {
    ...
    std::cout<<"programe run here"<<std::endl;
    // GENERAL CONFIG
...
//  descartes_light::KinematicsInterfaceD::Ptr kin_interface =std::make_shared<ur_ikfast_kinematics::Fanucm20iaKinematicsD>(world_to_base_link,tool0_to_sander,isValidFn,
//                                                                        nullptr);
